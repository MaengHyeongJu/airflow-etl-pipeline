...
성남 **해오름도서관 + 중원어린이도서관** 신착도서(문학/최근 3개월/일반도서) 중 **대출가능한 SF·추리소설**만 매일 찾아주는 개인용 DAG. 대상 도서관은 `airflow/dags/library_new_arrivals_dag.py`의 `TARGET_LIBRARIES` 리스트(도서관 코드는 `library_scanner/snlib_client.py`의 `LIBRARY_CODES` 참고)에 추가하면 더 늘릴 수 있다.

```
scrape_and_stage (도서관별로 snlib.go.kr 스크래핑 후 병합)
    -> load_raw (Postgres library.new_arrival_scan, 일자 파티션 덮어쓰기)
    -> classify_and_load (알라딘 ISBN 조회 -> categoryName으로 SF/추리 판별, 줄거리도 함께 수집 -> library.genre_matches)
...
- **결과 확인 방법 두 가지**: ① Airflow UI에서 `classify_and_load` 태스크 로그 → `[장르] 제목 / 저자 (도서관) - 카테고리: 줄거리` 형식으로 한 줄씩 남음 ② `data/reports/library/latest.txt` — 날짜 몰라도 바로 열어보는 최신 결과 텍스트 파일(도서관명·청구기호·줄거리 포함)
...
- 독립 실행/테스트: `python -m library_scanner.cli --library MH --kdc 8 --period 3M --classify` (16개 유닛테스트는 `cd library_scanner && python3 -m pytest`)
- **`library_scanner/`는 Docker 이미지 빌드 시점에 COPY되는 패키지라 `airflow/dags`·`airflow/include`와 달리 바인드 마운트가 아님.** 이 패키지 코드를 고치면 `docker compose up -d --build airflow-scheduler airflow-dag-processor airflow-api-server airflow-triggerer`로 이미지를 재빌드해야 반영된다.
