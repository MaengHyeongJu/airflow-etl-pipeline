"""성남도서관(해오름도서관 + 중원어린이도서관) 신착도서 중 대출가능한 SF/추리 소설 찾기.

    scrape_and_stage >> load_raw_scan >> classify_and_load_matches >> write_report

- scrape_and_stage: snlib.go.kr 신착도서 검색(도서관별/문학/3개월/일반도서)을
  도서관마다 스크래핑해서 로컬 staging 파일에 합쳐 저장. robots.txt가 사이트
  전체를 막아둬서 하루 한 번(스케줄), 페이지당 딜레이를 두고 저빈도로만 호출한다.
...
TARGET_LIBRARIES = ["MH", "CK"]  # 해오름도서관, 중원어린이도서관
TARGET_KDC = "8"  # 문학
...
def library_new_arrivals_scan():
    ...
    def scrape_and_stage(ds: str) -> str:
        return scrape_to_staging(
            ds, libraries=TARGET_LIBRARIES, kdc=TARGET_KDC, period=TARGET_PERIOD, book_class=TARGET_BOOK_CLASS
        )
